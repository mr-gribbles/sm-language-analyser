...
from sklearn.metrics import (
    accuracy_score, auc, balanced_accuracy_score, brier_score_loss,
...
class ComprehensiveModelEvaluator:
            ...
            if model_type in sklearn_types:
                ...
                try:
                    ...
                    class NeuralNetworkWrapper:
                        ...
                        def fit(self, X, y):
                            """Dummy fit method for cross-validation compatibility"""
                            return self
                    ...
                    return NeuralNetworkWrapper(pytorch_model), model_type, feature_extractor
    ...
    def calculate_core_classification_metrics(self, y_true: np.ndarray, y_pred: np.ndarray,
        ...
        metrics['precision_class_0'] = precision_per_class[0] if len(precision_per_class) > 0 else 0.0
        metrics['precision_class_1'] = precision_per_class[1] if len(precision_per_class) > 1 else 0.0
        metrics['recall_class_0'] = recall_per_class[0] if len(recall_per_class) > 0 else 0.0
        metrics['recall_class_1'] = recall_per_class[1] if len(recall_per_class) > 1 else 0.0
    ...
    def calculate_bootstrap_confidence_intervals(self, y_true: np.ndarray, y_pred: np.ndarray,
    ...
    def calculate_error_analysis_metrics(self, y_true: np.ndarray, y_pred: np.ndarray,
                                       y_prob: Optional[np.ndarray] = None) -> Dict[str, float]:
        ...
        try:
            ...
            if y_prob is not None:
                ...
                high_confidence_mask = (np.abs(y_prob - 0.5) > 0.3)  # >80% or <20% confidence
                ...
                if correct_predictions.any():
                    metrics['avg_confidence_correct'] = np.abs(y_prob[correct_predictions] - 0.5).mean()
                else:
                    metrics['avg_confidence_correct'] = 0.0
                    
                if incorrect_predictions.any():
                    metrics['avg_confidence_incorrect'] = np.abs(y_prob[incorrect_predictions] - 0.5).mean()
                else:
    ...
    def calculate_business_metrics(self, model, model_name: str, X: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            metrics['throughput_samples_per_second'] = 1.0 / inference_time_per_sample if inference_time_per_sample > 0 else 0
        ...
        try:
            ...
            if feature_extractor is not None and feature_extractor != self.feature_extractor:
                ...
                if self.original_texts:
                    try:
                        ...
                        print(f"  Warning: Could not re-extract features for {model_name}: {e}")
                        X_for_model = self.X_test
            ...
            if hasattr(model, 'predict_proba'):
                ...
                try:
                    ...
                    print(f"  Warning: Could not get decision function from {model_name}: {e}")
            
            # Calculate all comprehensive metrics
            core_metrics = self.calculate_core_classification_metrics(self.y_test, y_pred, y_pred_proba)
            results.update(core_metrics)
            
            # Calibration metrics
            if y_pred_proba is not None:
                calibration_metrics = self.calculate_calibration_metrics(self.y_test, y_pred_proba)
                results.update(calibration_metrics)
            
            # Robustness metrics (simplified for performance)
            try:
                robustness_metrics = self.calculate_robustness_metrics(model, X_for_model, self.y_test, cv_folds=3)
                results.update(robustness_metrics)
            except Exception as e:
                print(f"  Warning: Could not calculate robustness metrics for {model_name}: {e}")
                results.update({
            ...
            try:
                ...
                print(f"  Warning: Could not calculate confidence intervals for {model_name}: {e}")
                results.update({
                    'accuracy_ci': (0.0, 1.0),
            ...
            # Error analysis
            error_metrics = self.calculate_error_analysis_metrics(self.y_test, y_pred, y_pred_proba)
            results.update(error_metrics)
            
            # Business metrics
            business_metrics = self.calculate_business_metrics(model, model_name, X_for_model)
    ...
    def _get_default_results(self, base_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        defaults = {
            ...
            'matthews_corrcoef': 0.0, 'cohen_kappa': 0.0,
            'auc_roc': 0.5, 'auc_pr': 0.5, 'brier_score': 0.25,
            'log_loss': 0.693, 'average_precision': 0.5,
            'expected_calibration_error': 0.0, 'reliability': 1.0,
            'cv_accuracy_mean': 0.0, 'cv_accuracy_std': 0.0, 'stability_score': 0.0,
            'total_errors': 0, 'error_rate': 0.0,
            'inference_time_per_sample_ms': 1.0, 'throughput_samples_per_second': 1000.0,
            'estimated_model_size_mb': 1.0, 'model_complexity_score': 0.5
        }
    ...
    def create_comprehensive_plots(self, df: pd.DataFrame) -> None:
        ...
        fig1.suptitle('Comprehensive Model Performance Analysis - Core Metrics', fontsize=16, fontweight='bold')
        ...
        if 'accuracy_ci' in df.columns:
            try:
                ci_lower = [eval(ci)[0] if isinstance(ci, str) else ci[0] for ci in top_models['accuracy_ci']]
                ci_upper = [eval(ci)[1] if isinstance(ci, str) else ci[1] for ci in top_models['accuracy_ci']]
                errors = [top_models['accuracy'].iloc[i] - ci_lower[i] for i in range(len(ci_lower))], \
                        [ci_upper[i] - top_models['accuracy'].iloc[i] for i in range(len(ci_upper))]
            except:
                errors = None
        else:
            errors = None
        
        bars = ax.barh(range(len(top_models)), top_models['accuracy'], xerr=errors, capsize=3)
        ...
        if 'f1_macro' in df.columns and 'f1_weighted' in df.columns:
            scatter = ax.scatter(df['f1_macro'], df['f1_weighted'], 
                               c=df['accuracy'], s=50, alpha=0.7, cmap='viridis')
        ...
        if 'auc_roc' in df.columns and 'auc_pr' in df.columns:
            scatter = ax.scatter(df['auc_roc'], df['auc_pr'], 
                               c=df['matthews_corrcoef'], s=50, alpha=0.7, cmap='RdYlGn')
        ...
        if 'expected_calibration_error' in df.columns:
            ax.hist(df['expected_calibration_error'], bins=20, alpha=0.7, edgecolor='black')
            ax.set_xlabel('Expected Calibration Error')
            ax.set_ylabel('Number of Models')
            ax.set_title('Model Calibration Distribution')
            ax.axvline(df['expected_calibration_error'].mean(), color='red', linestyle='--', 
        ...
        if 'cv_accuracy_std' in df.columns:
            scatter = ax.scatter(df['cv_accuracy_std'], df['accuracy'], 
                               c=df['stability_score'], s=50, alpha=0.7, cmap='RdYlGn')
        ...
        if len(df.groupby('model_type')) > 1:
            ...
            for i, (bar, count) in enumerate(zip(bars, type_stats[('accuracy', 'count')])):
                height = bar.get_height()
                ax.text(bar.get_x() + bar.get_width()/2., height + acc_stds.iloc[i],
        ...
        fig2.suptitle('Business and Practical Performance Analysis', fontsize=16, fontweight='bold')
        
        # 2.1: Performance vs Inference Speed
        ax = axes2[0, 0]
        if 'throughput_samples_per_second' in df.columns:
            scatter = ax.scatter(np.log10(df['throughput_samples_per_second']), df['accuracy'], 
                               c=df['estimated_model_size_mb'], s=50, alpha=0.7, cmap='viridis')
        ...
        if 'estimated_model_size_mb' in df.columns:
            scatter = ax.scatter(df['estimated_model_size_mb'], df['accuracy'], 
                               c=df['model_complexity_score'], s=50, alpha=0.7, cmap='RdYlBu')
        ...
        if 'high_confidence_error_rate' in df.columns:
            ...
            # Add diagonal line
            max_error = max(df['error_rate'].max(), df['high_confidence_error_rate'].max())
            ax.plot([0, max_error], [0, max_error], 'r--', alpha=0.5)
        ax.grid(True, alpha=0.3)
        
        # 2.4: Metric correlation heatmap
        ax = axes2[1, 0]
        correlation_metrics = ['accuracy', 'f1_weighted', 'auc_roc', 'matthews_corrcoef', 
        ...
        metrics_for_comparison = ['accuracy', 'f1_weighted', 'auc_roc', 'reliability', 'stability_score']
        available_comparison_metrics = [m for m in metrics_for_comparison if m in df.columns]
        ...
        if len(available_comparison_metrics) > 2 and len(top_5_models) > 0:
            ...
            for i, (_, model) in enumerate(top_5_models.iterrows()):
                ...
                ax.bar(x_pos + i*width, values, width, 
                      label=model['method'][:15], alpha=0.8)
    ...
    def generate_comprehensive_report(self, df: pd.DataFrame) -> None:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write("This report provides a detailed analysis of all evaluated text classification models ")
            f.write("using industry-standard metrics and best practices.\n\n")
            
            f.write("## Executive Summary\n\n")
            f.write(f"- **Total Models Evaluated**: {len(df)}\n")
            f.write(f"- **Best Overall Model**: {df.loc[df['accuracy'].idxmax(), 'method']}\n")
            ...
            f.write(f"- **Most Stable Model**: {df.loc[df['stability_score'].idxmax(), 'method'] if 'stability_score' in df.columns else 'N/A'}\n\n")
            
            f.write("## Core Performance Metrics\n\n")
            f.write("### Top 10 Models by Accuracy\n\n")
            top_models = df.nlargest(10, 'accuracy')
            f.write("| Rank | Model | Accuracy | F1 (Weighted) | AUC-ROC | Matthews Corr |\n")
            f.write("|------|-------|----------|---------------|---------|---------------|\n")
            for i, (_, model) in enumerate(top_models.iterrows(), 1):
                f.write(f"| {i:2d} | {model['method'][:25]:<25} | {model['accuracy']:.4f} | ")
                f.write(f"{model.get('f1_weighted', 0):.4f} | {model.get('auc_roc', 0.5):.4f} | ")
                f.write(f"{model.get('matthews_corrcoef', 0):.4f} |\n")
            
            f.write("\n### Performance Distribution\n\n")
            f.write(f"- **Accuracy**: μ={df['accuracy'].mean():.4f}, σ={df['accuracy'].std():.4f}\n")
            f.write(f"- **F1 Score (Weighted)**: μ={df.get('f1_weighted', pd.Series([0])).mean():.4f}, ")
            f.write(f"σ={df.get('f1_weighted', pd.Series([0])).std():.4f}\n")
            f.write(f"- **AUC-ROC**: μ={df.get('auc_roc', pd.Series([0.5])).mean():.4f}, ")
            ...
            if 'model_type' in df.columns:
                ...
                f.write("| Model Type | Count | Mean Acc | Std Dev | Min Acc | Max Acc |\n")
                f.write("|------------|-------|----------|---------|---------|----------|\n")
                for model_type, stats in type_stats.iterrows():
                    f.write(f"| {model_type:<10} | {int(stats[('accuracy', 'count')]):>5} | ")
                    f.write(f"{stats[('accuracy', 'mean')]:>8.4f} | {stats[('accuracy', 'std')]:>7.4f} | ")
                    f.write(f"{stats[('accuracy', 'min')]:>7.4f} | {stats[('accuracy', 'max')]:>8.4f} |\n")
            ...
            if 'expected_calibration_error' in df.columns:
                ...
                f.write(f"- **Well-calibrated models** (ECE < 0.05): {well_calibrated}/{len(df)}\n")
                f.write(f"- **Average Expected Calibration Error**: {df['expected_calibration_error'].mean():.4f}\n")
                f.write(f"- **Best calibrated model**: {df.loc[df['expected_calibration_error'].idxmin(), 'method']}\n\n")
            
            # Robustness Analysis
            if 'cv_accuracy_std' in df.columns:
                f.write("### Model Robustness\n\n")
                stable_models = len(df[df['cv_accuracy_std'] < 0.02])
                f.write(f"- **Stable models** (CV std < 0.02): {stable_models}/{len(df)}\n")
                f.write(f"- **Average CV accuracy std**: {df['cv_accuracy_std'].mean():.4f}\n")
                f.write(f"- **Most stable model**: {df.loc[df['cv_accuracy_std'].idxmin(), 'method']}\n\n")
            
            # Business Metrics
            if 'throughput_samples_per_second' in df.columns:
                f.write("### Business Performance\n\n")
                f.write(f"- **Fastest model**: {df.loc[df['throughput_samples_per_second'].idxmax(), 'method']} ")
                f.write(f"({df['throughput_samples_per_second'].max():.0f} samples/sec)\n")
                f.write(f"- **Average throughput**: {df['throughput_samples_per_second'].mean():.0f} samples/sec\n")
                
                if 'estimated_model_size_mb' in df.columns:
                    f.write(f"- **Smallest model**: {df.loc[df['estimated_model_size_mb'].idxmin(), 'method']} ")
                    f.write(f"({df['estimated_model_size_mb'].min():.1f} MB)\n")
                    f.write(f"- **Average model size**: {df['estimated_model_size_mb'].mean():.1f} MB\n\n")
            ...
            if 'throughput_samples_per_second' in df.columns:
                ...
                if not good_performers.empty:
                    fastest_good = good_performers.loc[good_performers['throughput_samples_per_second'].idxmax()]
                    f.write(f"\n### Best Performance/Speed Trade-off: {fastest_good['method']}\n")
                    f.write(f"- **Accuracy**: {fastest_good['accuracy']:.4f}\n")
                    f.write(f"- **Throughput**: {fastest_good['throughput_samples_per_second']:.0f} samples/sec\n")
            
            if 'cv_accuracy_std' in df.columns:
                good_performers = df[df['accuracy'] > df['accuracy'].quantile(0.75)]
                if not good_performers.empty:
                    most_stable = good_performers.loc[good_performers['cv_accuracy_std'].idxmin()]
            ...
            f.write("- Confidence intervals calculated using bootstrap sampling (n=100)\n")
            f.write("- Cross-validation performed with 3-fold stratified sampling\n")
            f.write("- Calibration measured using Expected Calibration Error (10 bins)\n")
            ...
            f.write("- **Precision/Recall/F1**: Per-class and averaged (macro/micro/weighted)\n")
            f.write("- **Specificity/Sensitivity**: True negative and positive rates\n")
            f.write("- **Matthews Correlation**: Correlation between predictions and true labels\n")
            f.write("- **Cohen's Kappa**: Inter-rater agreement accounting for chance\n\n")
            
            f.write("### Probability-Based Metrics:\n")
            f.write("- **AUC-ROC**: Area under ROC curve\n")
            f.write("- **AUC-PR**: Area under Precision-Recall curve\n")
            f.write("- **Brier Score**: Mean squared difference between probabilities and outcomes\n")
            ...
            f.write("- **Expected Calibration Error**: Difference between confidence and accuracy\n")
            f.write("- **Stability Score**: Model consistency across cross-validation folds\n")
            f.write("- **Bootstrap Confidence Intervals**: Statistical uncertainty estimates\n\n")
    ...
    def run_comprehensive_evaluation(self) -> None:
        ...
        print(f"✓ Total metrics per model: {len(df.columns) - 3}")  # Subtract meta columns
        ...
        print(f"  • business_practical_analysis.png") 
        print(f"  • comprehensive_model_evaluation_report.md")
