...
class FeatureExtractor:
    ...
    def __init__(
        ...
        if ENHANCED_EXTRACTOR_AVAILABLE:
            try:
                self.enhanced_extractor = create_enhanced_feature_extractor(
                    "baseline_plus"
                )
    ...
    def _fit_transform_basic(self, texts: List[str]) -> np.ndarray:
        """Fit and transform using basic feature extraction (fallback)."""
...
class OptimizedNeuralNetwork(nn.Module):
    """Optimized neural network architectures from create_neural_ensemble.py."""

    def __init__(self, input_dim: int, architecture_type: str = "best_ensemble"):
        """Initialize the optimized neural network."""
        ...
        if architecture_type == "best_ensemble":
            ...
            # This architecture has a custom forward pass
            self._create_wide_deep_hybrid_architecture(input_dim)
            self.network = None
        else:
            self.network = self._create_optimized_architecture(input_dim)
...
class ModelPredictor:
    ...
    def predict_single_model(
        self, model_data: Dict[str, Any], text: str, model_name: str = None
    ) -> Tuple[int, float]:
        ...
        try:
            ...
            if (
                hasattr(feature_extractor, "enhanced_extractor")
                and feature_extractor.enhanced_extractor
            ):
                try:
            ...
            if model_type == "neural_network":
                ...
                return self._predict_neural_network_ensemble(
                    model_data, features, model_name
                )
            else:
                # Handle all sklearn-compatible model types
                return self._predict_sklearn_model(model_data, features, model_name)
    ...
    def _predict_neural_network(
        self, model_data: Dict[str, Any], features: np.ndarray, model_name: str = None
    ) -> Tuple[int, float]:
        ...
        try:
            ...
            if model_name == "neural_network_wide_deep_hybrid":
                ...
                return self._predict_best_ensemble_architecture(
                    model_data, features, model_name
                )
    ...
    def _predict_wide_deep_hybrid(
        self, model_data: Dict[str, Any], features: np.ndarray, model_name: str
    ) -> Tuple[int, float]:
        ...
        try:
            ...
            model = NeuralNetwork(
                input_dim, hidden_dims=[1024, 512, 256], dropout=0.3, activation="relu"
            )
    ...
    def _predict_best_ensemble_architecture(
        self, model_data: Dict[str, Any], features: np.ndarray, model_name: str
    ) -> Tuple[int, float]:
        ...
        try:
            ...
            # Use the OptimizedNeuralNetwork class which contains the correct architecture
            model = OptimizedNeuralNetwork(input_dim, architecture_type="best_ensemble")
    ...
    def _predict_neural_network_ensemble(
        self, model_data: Dict[str, Any], features: np.ndarray, model_name: str = None
    ) -> Tuple[int, float]:
        ...
        try:
            ...
            for i in range(ensemble_size):
                ...
                # Create model using the correct OptimizedNeuralNetwork class and architecture type
                model = OptimizedNeuralNetwork(input_dim, architecture_type=arch).to(
                    device
                )
    ...
    def _predict_sklearn_model(
        ...
        try:
            ...
            if isinstance(model, (MultinomialNB, ComplementNB)):
                ...
                try:
                    ...
                    if "features" in str(dim_error) and "expecting" in str(dim_error):
                        ...
                        if feature_extractor and hasattr(
                            feature_extractor, "max_features"
                        ):
                            try:
                                word_features_count = min(
                                    features.shape[1],
                                    feature_extractor.max_features // 2,
                                )
            ...
            try:
                ...
                probability = 0.9 if prediction == 1 else 0.1
                print(
                    f"Warning: Probability calculation failed for {model_name}: {prob_error}"
                )
    ...
    def predict_all_models(
        ...
        if len(text.strip()) < 10:
            raise ValueError(
                "Text too short for reliable classification (minimum 10 characters)"
            )
        ...
        for model_file in sorted(model_files):
            ...
            try:
                ...
                # Make prediction
                prediction, probability = self.predict_single_model(
                    model_data, text, model_name
                )
    ...
    def interactive_mode(self):
        ...
        while True:
            try:
                ...
                print(
                    f"Vote Distribution: {details['ai_predictions']} AI, {details['human_predictions']} Human"
                )
                print(
                    f"Margin: {details['margin']} models ({details['margin_percentage']:.1f}%)"
                )
                ...
                ai_models = [
                    (name, prob)
                    for name, (pred, prob) in individual.items()
                    if pred == 1
                ]
                human_models = [
                    (name, prob)
                ...
                if human_models:
                    ...
                    for name, prob in human_models:
                        confidence = (
                            1 - prob
                        )  # Convert AI probability to Human confidence
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AI vs Human text classifier prediction"
    )
    parser.add_argument("--text", type=str, help="Single text to classify")
    parser.add_argument(
        "--file", type=str, help="File containing texts to classify (one per line)"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Run in interactive mode"
    )
    parser.add_argument(
        "--models-dir", default="models", help="Directory containing trained models"
    )
    parser.add_argument(
        "--output", type=str, help="Output file for batch predictions (CSV format)"
    )
    ...
    try:
        if args.interactive:
            ...
            if args.model:
                ...
                prediction, probability = predictor.predict_single_model(
                    model_data, args.text
                )
                label = "AI-Generated" if prediction == 1 else "Human-Written"
                ...
                # Use ensemble
                prediction, confidence, details = predictor.get_ensemble_prediction(
                    args.text
                )
                label = "AI-Generated" if prediction == 1 else "Human-Written"
                ...
                print(f"Confidence: {confidence:.3f}")
                print(
                    f"Vote Distribution: {details['ai_predictions']} AI, "
                    f"{details['human_predictions']} Human"
                )
            ...
            print(
                f"Predicted as AI: {len(results_df[results_df['prediction'] == 'AI'])}"
            )
            print(
                f"Predicted as Human: {len(results_df[results_df['prediction'] == 'Human'])}"
            )
