...
class ModelPredictor:
    ...
    def _predict_sklearn_model(self, model_data: Dict[str, Any], features: np.ndarray, text: str) -> Tuple[int, float]:
        """Predict using scikit-learn model."""
        try:
            model = model_data['model']
            
            # Handle MultinomialNB special case (needs only word features)
            from sklearn.naive_bayes import MultinomialNB
            if isinstance(model, MultinomialNB):
                # Extract word features only (first half of features)
                feature_extractor = model_data['feature_extractor']
                word_features_count = feature_extractor.max_features // 2
