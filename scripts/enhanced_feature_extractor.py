...
from sklearn.feature_selection import (
    SelectKBest, SelectPercentile, chi2, f_classif,
...
class AdvancedLinguisticFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, include_readability=True, include_pos_tags=True, 
                 include_sentiment=True, include_stylometric=True):
        ...
        self.patterns = {
            ...
            'punctuation_clusters': re.compile(r'[{}]{{2,}}'.format(re.escape(string.punctuation))),
        }
    ...
    def _extract_character_features(self, text: str) -> List[float]:
        ...
        return [
            upper_ratio, lower_ratio, digit_ratio, alpha_ratio,
            space_ratio, punct_ratio, question_ratio, exclaim_ratio,
            period_ratio, comma_ratio, quote_ratio, char_diversity
        ]
    ...
    def _extract_word_features(self, text: str) -> List[float]:
        ...
        return [
            ...
            lexical_diversity, long_word_ratio, short_word_ratio,
            function_word_ratio, capitalized_ratio, unique_words.__len__()
        ]
    ...
    def _extract_sentence_features(self, text: str) -> List[float]:
        ...
        return [
            sentence_count, avg_sentence_len, std_sentence_len,
            max_sentence_len, min_sentence_len, avg_words_per_sentence,
            question_ratio, exclaim_ratio
        ]
    ...
    def _extract_readability_features(self, text: str) -> List[float]:
        ...
        ari_score = (4.71 * (char_count / word_count) + 
    ...
    def _extract_stylometric_features(self, text: str) -> List[float]:
        ...
        V1 = hapax_count
        if N > 0 and V1 > 0:
            yules_k = 10000 * (sum(count * count for count in word_freq.values()) - N) / (N**2)
        else:
            yules_k = 0
            
        # Simpson's D (vocabulary diversity)
        if N > 1:
            simpsons_d = sum(count * (count - 1) for count in word_freq.values()) / (N * (N - 1))
        else:
            simpsons_d = 0
        ...
        return [
            most_freq_ratio, hapax_ratio, dis_ratio, yules_k, simpsons_d,
            honores_r, brunets_w, ttr, rttr, cttr, mtld_approx, avg_word_freq
        ]
    ...
    def get_feature_names(self) -> List[str]:
        ...
        # Basic stats
        names.extend(['text_len', 'char_count', 'word_count', 'char_density', 
                     'word_density', 'avg_word_len_chars'])
        
        # Character features
        names.extend(['upper_ratio', 'lower_ratio', 'digit_ratio', 'alpha_ratio',
                     'space_ratio', 'punct_ratio', 'question_ratio', 'exclaim_ratio',
                     'period_ratio', 'comma_ratio', 'quote_ratio', 'char_diversity'])
        
        # Word features
        names.extend(['word_avg_len', 'word_std_len', 'word_max_len', 'word_min_len',
                     'lexical_diversity', 'long_word_ratio', 'short_word_ratio',
                     'function_word_ratio', 'capitalized_ratio', 'unique_word_count'])
        
        # Sentence features
        names.extend(['sentence_count', 'sentence_avg_len', 'sentence_std_len',
                     'sentence_max_len', 'sentence_min_len', 'avg_words_per_sentence',
                     'question_sentence_ratio', 'exclaim_sentence_ratio'])
        
        # Social media features
        names.extend(['urls_per_1k', 'emails_per_1k', 'mentions_per_1k', 'hashtags_per_1k',
                     'repeated_chars_per_1k', 'repeated_words_per_1k', 'all_caps_per_1k',
                     'punct_clusters_per_1k'])
        
        if self.include_readability:
            names.extend(['flesch_score', 'grade_level', 'ari_score', 'avg_syllables_per_word'])
        
        if self.include_stylometric:
            names.extend(['most_freq_ratio', 'hapax_ratio', 'dis_ratio', 'yules_k', 'simpsons_d',
                         'honores_r', 'brunets_w', 'ttr', 'rttr', 'cttr', 'mtld_approx', 'avg_word_freq'])
...
class EnhancedFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        feature_union = FeatureUnion(feature_extractors, n_jobs=-1)
        ...
        if self.select_k_best is not None:
            ...
            self.feature_selector = SelectKBest(
                ...
                k=min(self.select_k_best, self.max_word_features + self.max_char_features)
            )
        ...
        temp_features = initial_pipeline.transform(X[:10])  # Small sample to check dimensions
        ...
        if (self.use_svd or self.use_pca) and n_features_after_selection > 1:
            if self.use_svd:
                ...
                n_components = min(
                    ...
                    max(1, n_features_after_selection // 2)  # Use at most half the features
                )
                if n_components > 0:
    ...
    def _print_feature_info(self, X):
        ...
        # Initial features
        temp_features = self.pipeline.named_steps['features'].transform(X[:100])  # Small sample
        ...
        if 'variance_selection' in self.pipeline.named_steps:
            temp_features = self.pipeline.named_steps['variance_selection'].transform(temp_features)
        ...
        if 'feature_selection' in self.pipeline.named_steps:
            temp_features = self.pipeline.named_steps['feature_selection'].transform(temp_features)
        ...
        if 'dimensionality_reduction' in self.pipeline.named_steps:
            temp_features = self.pipeline.named_steps['dimensionality_reduction'].transform(temp_features)
            feature_counts.append(f"After dimensionality reduction: {temp_features.shape[1]}")
...
if __name__ == "__main__":
    ...
    sample_texts = [
        ...
        "Human writing often contains informal elements, contractions, and varied sentence structures."
    ]
