...
from sklearn.metrics import (accuracy_score, balanced_accuracy_score,
                             brier_score_loss, cohen_kappa_score, log_loss,
                             matthews_corrcoef,
...
try:
    ...
    print(
        "Successfully imported FeatureExtractor and NeuralNetwork " "from train_models"
    )
...
class NewModelEvaluator:
    ...
    def load_model_safely(self, model_path: Path) -> Tuple[Any, str, Any]:
        ...
        try:
            ...
            if model_type in sklearn_types:
                ...
                or model_type == "neural_network_ensemble"
            ):
                ...
                try:
                    ...
                    if model_type == "neural_network_ensemble":
                        print(
                            f"Loading ensemble with {model_data.get('ensemble_size', 0)} models"
                        )
                        # For now, just use the first model in the ensemble
                        model_states = model_data.get("model_states", [])
                        if not model_states:
                            print(
                                f"Warning: No model_states found in ensemble {model_path.name}"
                            )
                            return None, model_type, feature_extractor
                    ...
                    hidden_sizes, max_layer = detect_architecture_from_state_dict(
                        model_state_dict
                    )
    ...
    def calculate_model_complexity_metrics(
        self, model, model_name: str, model_type: str
    ) -> Dict[str, float]:
        ...
        if any(x in method for x in ["neural", "deep", "mlp", "transformer"]):
            ...
            x in method for x in ["svm", "gradient", "xgboost", "lightgbm", "catboost"]
        ):
    ...
    def calculate_composite_scores(self, metrics: Dict[str, float]) -> Dict[str, float]:
        ...
        composite_scores["performance_score"] = (
            0.3 * accuracy + 0.3 * f1_weighted + 0.2 * auc_roc + 0.2 * matthews_corrcoef
        )
        ...
        composite_scores["efficiency_score"] = efficiency_raw * (
            1.0 - model_complexity * 0.3
        )
    ...
    def evaluate_single_model(
        ...
        try:
            ...
            if (
                ...
                # This model has its own feature extractor, need to re-extract
                ...
                if texts:
                    ...
                    print(
                        f"  Warning: Could not get original texts for {model_name}, "
                        "using shared features"
                    )
                    X_for_model = self.X_test
                    print(
                        "  Re-extracted features: {} features for {} samples".format(
                            X_for_model.shape[1], X_for_model.shape[0]
                        )
                    )
            ...
            complexity_metrics = self.calculate_model_complexity_metrics(
    ...
    def create_performance_plots(self, df: pd.DataFrame) -> None:
        ...
        ax1.barh(range(len(top_models)), top_models["accuracy"])
        ...
        ax3 = axes[0, 2]
        type_performance = (
            df.groupby("model_type")["accuracy"].agg(["mean", "std"]).reset_index()
        )
        ax3.bar(
            type_performance["model_type"],
            type_performance["mean"],
            yerr=type_performance["std"],
            capsize=5,
            alpha=0.7,
        )
        ...
        if all(col in df.columns for col in metrics_cols):
            ...
            ax5.set_xticklabels(
                top_10_models["method"].str[:10], rotation=45, ha="right", fontsize=8
            )
        ...
        ax6.bar(range(len(quality_sorted)), quality_sorted["quality_score"])
        ax6.set_xticks(range(len(quality_sorted)))
        ax6.set_xticklabels(
            quality_sorted["method"], rotation=45, ha="right", fontsize=8
        )
    ...
    def generate_summary_report(self, df: pd.DataFrame) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            for i, (_, model) in enumerate(quality_models.iterrows(), 1):
                f.write(
                    f"{i:2d}. {model['method']:<25}: {model['quality_score']:.4f}\n"
                )
            ...
            f.write("-" * 25 + "\n")
            type_stats = (
                df.groupby("model_type")["accuracy"]
                .agg(["count", "mean", "std"])
                .round(4)
            )
            for model_type, model_stats in type_stats.iterrows():
                f.write(
                    f"{model_type:<15}: {model_stats['count']:>3} models, "
                    f"avg: {model_stats['mean']:.4f}, std: {model_stats['std']:.4f}\n"
                )
            ...
            f.write(
                f"• Best performing model: {best_model['method']} ({best_model['accuracy']:.4f})\n"
            )

            if "sklearn" in df["model_type"].values:
                best_sklearn = df[df["model_type"] == "sklearn"].loc[
                    df[df["model_type"] == "sklearn"]["accuracy"].idxmax()
                ]
                f.write(
                    f"• Best sklearn model: {best_sklearn['method']} ({best_sklearn['accuracy']:.4f})\n"
                )

            f.write(
                f"• Average model complexity: {df['model_complexity'].mean():.3f}\n"
            )
...
def main():
    """Run the main function to run model evaluation."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Evaluate models trained with train_models.py"
    )
    parser.add_argument(
        "--models-dir",
        type=str,
