        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for i, line in enumerate(f):
                try:
                    ...
                    if record.get("llm_transformation") and record[
                        "llm_transformation"
                    ].get("rewritten_text"):
                        ...
                        text_to_analyze = original_content.get(
                            "cleaned_selftext"
                        ) or original_content.get("cleaned_text")
                        if i == 0:
                            analysis_type = "Original Text"

                    analysis_results = (
                        run_full_analysis(text_to_analyze) if text_to_analyze else {}
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "filepath", type=str, help="The path to the .jsonl corpus file to analyze."
    )
