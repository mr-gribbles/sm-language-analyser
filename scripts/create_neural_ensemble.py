...
try:
    ...
    print("Warning: Could not import from train_models")
...
class NeuralNetworkEnsemble:
    ...
    def create_models(self):
        ...
        print(f"Created ensemble of {len(self.models)} optimized networks")
    ...
    def train_ensemble(
        ...
        for i, model in enumerate(self.models):
            arch_name = self.architectures[i % len(self.architectures)]
            print(f"\nTraining model {i+1}/{len(self.models)} ({arch_name})...")
            ...
            for epoch in range(epochs):
                ...
                if (epoch + 1) % 50 == 0:
                    print(
                        f"    Epoch {epoch+1}: train_loss={loss:.4f}, val_loss={val_loss:.4f}"
                    )
    ...
    if os.path.exists(human_file):
        with open(human_file, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                except (json.JSONDecodeError, KeyError) as e:  # noqa: F841
                    if line_num <= 10:
                        print(
...
def create_and_train_ensemble():
    """Create and train the neural network ensemble."""
    print("=" * 80)
