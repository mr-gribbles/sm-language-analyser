    ...
    for attempt in range(max_retries):
        try:
            ...
            for item in response.feed:
                ...
                if not (
                    isinstance(post.record, Record)
                    and post.record.text
                    and "en" in (post.record.langs or [])
                ):
                    continue
            ...
            if e.response and e.response.status_code == 400:
                print(
                    "Fatal: Received a 400 Bad Request error from Bluesky. "
            ...
            print(
                f"Warning: An error occurred while fetching from Bluesky. "
