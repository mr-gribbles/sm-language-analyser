...
def clean_text(raw_text: str) -> str:
    ...
    text = re.sub(r'\b[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}(/\S*)?\b', '', text, flags=re.MULTILINE)
    
    # Remove any remaining URL-like patterns (belt and suspenders approach)
    text = re.sub(r'\S+\.(com|org|net|edu|gov|io|co|uk|de|fr|jp|au|ca|in|it|ru|br|mx|es|nl|se|no|dk|fi|pl|be|at|ch|cz|ie|pt|gr|hu|ro|bg|hr|sk|si|lt|lv|ee|lu|mt|cy)\S*', '', text, flags=re.MULTILINE)
