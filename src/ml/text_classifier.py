...
class TextClassifierNetwork(nn.Module):
    ...
    def __init__(self, input_dim: int, dropout_rate: float = 0.5):
        ...
        # Minimal architecture to prevent overfitting
...
class AIHumanTextClassifier:
    ...
    def extract_features(self, texts: List[str]) -> np.ndarray:
        ...
        if self.vectorizer is None:
            self.vectorizer = TfidfVectorizer(
                ...
                min_df=10,  # Very aggressive - need at least 10 occurrences
                max_df=0.7,  # Exclude very common terms (70%+)
    ...
    def _train_model(self, texts: List[str], labels: List[int], test_size: float = 0.2, 
                    ...
                    learning_rate: float = 0.0005, patience: int = 20, dropout_rate: float = 0.5,
        ...
        # Initialize model
        self.model = TextClassifierNetwork(features_scaled.shape[1], dropout_rate).to(self.device)
        
        # Use standard BCE loss with stronger regularization
        ...
        for epoch in range(epochs):
            ...
            for batch_features, batch_labels in train_loader:
                ...
                # Add small amount of noise for regularization during training
                ...
                feature_dropout_mask = torch.rand_like(batch_features) > 0.05  # Keep 95% of features
                batch_features = batch_features * feature_dropout_mask.float()
            ...
            # Stop if training accuracy gets too high (overfitting prevention)
            if train_acc > 0.92:
                print(f"Stopping training - train accuracy too high ({train_acc:.4f}) at epoch {epoch+1}")
                break
    ...
    def train(self, corpus_dir: str, test_size: float = 0.2, validation_size: float = 0.1, 
              ...
              patience: int = 10, dropout_rate: float = 0.5, weight_decay: float = 1e-4) -> Dict[str, Any]:
        ...
        return self._train_model(texts, labels, test_size, validation_size, epochs, 
                               batch_size, learning_rate, patience, dropout_rate, weight_decay)
    ...
    def train_from_files(self, human_file: str, ai_file: str, test_size: float = 0.2, 
                        ...
                        learning_rate: float = 0.001, patience: int = 10, dropout_rate: float = 0.5,
                        weight_decay: float = 1e-4) -> Dict[str, Any]:
    ...
    def load_model(self, model_path: str):
        ...
        # Load model
        checkpoint = torch.load(f"{model_path}_model.pth", map_location=self.device, weights_only=True)
