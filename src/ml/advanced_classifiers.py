...
class AdvancedTextClassifier:
    ...
    def save_model(self, model_path: str):
        ...
        # Save model(s)
        if self.is_anomaly_detector:
            joblib.dump(self.human_model, f"{model_path}_{self.classifier_type}_human_model.pkl")
            joblib.dump(self.ai_model, f"{model_path}_{self.classifier_type}_ai_model.pkl")
        else:
            joblib.dump(self.model, f"{model_path}_{self.classifier_type}_model.pkl")
        
        # Save preprocessing components
        with open(f"{model_path}_{self.classifier_type}_word_vectorizer.pkl", 'wb') as f:
            pickle.dump(self.word_vectorizer, f)
            
        with open(f"{model_path}_{self.classifier_type}_char_vectorizer.pkl", 'wb') as f:
            pickle.dump(self.char_vectorizer, f)
        
        with open(f"{model_path}_{self.classifier_type}_scaler.pkl", 'wb') as f:
        ...
        with open(f"{model_path}_{self.classifier_type}_config.json", 'w') as f:
            json.dump(config, f, indent=2)
        
        print(f"{self.classifier_type.title()} model saved to {model_path}_{self.classifier_type}_*.pkl")
    ...
    def load_model(self, model_path: str):
        ...
        self.is_anomaly_detector = config['is_anomaly_detector']
        
        # Load model(s)
        if self.is_anomaly_detector:
            self.human_model = joblib.load(f"{model_path}_{self.classifier_type}_human_model.pkl")
            self.ai_model = joblib.load(f"{model_path}_{self.classifier_type}_ai_model.pkl")
        else:
        ...
        print(f"{self.classifier_type.title()} model loaded from {model_path}")
